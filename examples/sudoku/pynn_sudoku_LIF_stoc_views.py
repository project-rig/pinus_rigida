...
weight_nois = 1.6                                                           # 1.6 for hard problem, 1.0 for dreams
delay       = 2.0                                                           # 2.0
...
cell_pop = p.Population(n_cell * 9 * 9,
                        p.IF_curr_exp(**cell_params_lif), label="Cells")

#
# add a noise source to each cell
#

noise_pop = p.Population(n_cell * 9 * 9,
                         p.SpikeSourcePoisson(rate=20.0, start=0.0, duration=dur_nois))

print("Creating views")

# Create cell views
cell = [[] for x, y in product(range(9), repeat=2)]
for x, y in product(range(9), repeat=2):
    cell_start = n_cell * (x + (y * 9))
    cell_stop = cell_start + n_cell

    cell
cell = [cell_pop[n_cell * (x + (y * 9)):n_cell + (n_cell * (x + (y * 9)))] for x, y in product(range(9), repeat=2)]

print("Creating noise projections")
syn_noise = p.StaticSynapse(weight=weight_nois)
conn_nois = p.OneToOneConnector(weight_nois)
p.Projection(noise_pop, cell_pop, conn_nois, syn_noise, receptor_type="excitatory")  # connect noise neurons one-to-one to cell neurons
